...
const api = {
  produk: {
    ...
    update: (id: number, data: ProdukInput) =>
      ipcRenderer.invoke("produk:update", id, data),
    ...
    exportCatalog: (selectedIds: number[]) =>
      ipcRenderer.invoke("produk:exportCatalog", selectedIds),
  },
  paket: {
    ...
    update: (id: number, data: PaketInput) =>
      ipcRenderer.invoke("paket:update", id, data),
  ...
  transaksi: {
    save: (input: SaveTransaksiInput) =>
      ipcRenderer.invoke("transaksi:save", input),
    list: (filter?: TransaksiFilter) =>
      ipcRenderer.invoke("transaksi:list", filter),
  ...
  laporan: {
    getSummary: (dateFrom?: string, dateTo?: string) =>
      ipcRenderer.invoke("laporan:getSummary", dateFrom, dateTo),
    getProdukProfit: (dateFrom?: string, dateTo?: string) =>
      ipcRenderer.invoke("laporan:getProdukProfit", dateFrom, dateTo),
    getItemTransactions: (
      itemId: number,
      tipe: "produk" | "paket",
      dateFrom?: string,
