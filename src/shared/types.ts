...
export interface LaporanProdukProfit {
  ...
  total_modal: number;
  total_profit: number;
  detail_kemasan: Record<
    string,
    { qty: number; subtotal: number; label: string }
  >;
}
...
export interface PosAPI {
  produk: {
    ...
    exportCatalog: (selectedIds: number[]) => Promise<boolean>;
  };
  paket: {
  ...
  transaksi: {
    save: (
      input: SaveTransaksiInput,
    ) => Promise<{ transaksi: Transaksi; receipt: string }>;
    list: (filter?: TransaksiFilter) => Promise<Transaksi[]>;
    get: (
      id: number,
    ) => Promise<{ transaksi: Transaksi; details: DetailTransaksi[] } | null>;
    getLast: () => Promise<{
      transaksi: Transaksi;
  ...
  laporan: {
    ...
    getProdukProfit: (
      dateFrom?: string,
      dateTo?: string,
    ) => Promise<LaporanProdukProfit[]>;
    getItemTransactions: (
      itemId: number,
      tipe: "produk" | "paket",
      dateFrom?: string,
      dateTo?: string,
    ) => Promise<LaporanItemTransaction[]>;
  };
  settings: {
